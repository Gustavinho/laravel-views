@props(['action', 'key', 'item', 'variant' => 'desktop'])

@if ($action->renderIf($item, $this))
  @switch($variant)
    ...
    'class' => 'border-2 border-transparent text-gray-600 rounded-full hover:text-gray-700 focus:outline-none focus:text-gray-700 focus:bg-gray-100 transition duration-150 ease-in-out',
        ...
        title="{{ $action->title }}" wire:click.prevent="executeAction('{{ $action->id() }}', '{{ $key }}')">
    ...
    @case('mobile')
      <button
        ...
        wire:click.prevent="executeAction('{{ $action->id() }}', '{{ $key }}')">
